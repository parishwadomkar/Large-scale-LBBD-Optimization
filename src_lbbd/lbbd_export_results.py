...
from .lbbd_export_common import (
...
def compute_lbbd_metrics(model, data: dict, cfg: dict, redir_rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def export_infrastructure_lbbd(model, data: dict, results_dir: Path) -> pd.DataFrame:
    rows = []
    for i in model.I:
        row = {"HexID": int(i)}
        for c in model.C_pub:
...
def export_energy_by_charger_type_lbbd(model, data: dict, redir_rows: list[dict[str, Any]], results_dir: Path) -> pd.DataFrame:
    incoming = _redir_incoming_by_slot_type(redir_rows)
    rows = []
    days = data["DAYS"]
    ndays = data["N_MONTH"]
...
def export_hourly_energy_lbbd(model, data: dict, redir_rows: list[dict[str, Any]], results_dir: Path) -> pd.DataFrame:
    incoming = _redir_incoming_by_slot_type(redir_rows)
...
def export_slack_lbbd(model, data: dict, results_dir: Path) -> pd.DataFrame:
    rows = []
    for i in model.I:
        for mon in model.M:
            for t in model.H:
...
def export_master_solution_lbbd(model, results_dir: Path) -> pd.DataFrame:
    rows = []
    for i in model.I:
        for c in model.C_pub:
...
def export_lbbd_results(model, data: dict, cfg: dict, run_dir: Path, redir_rows: list[dict[str, Any]], history_df: pd.DataFrame, sp_df: pd.DataFrame, cuts_df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    results_dir = run_dir / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    metrics = compute_lbbd_metrics(model, data, cfg, redir_rows)
    frames = {
        "model_summary": export_model_summary(metrics, results_dir),
        "infrastructure_by_hex": export_infrastructure_lbbd(model, data, results_dir),
        "energy_by_type": export_energy_by_charger_type_lbbd(model, data, redir_rows, results_dir),
        "origin_type_q": export_origin_type_allocation(model, data, results_dir),
        "hourly_energy": export_hourly_energy_lbbd(model, data, redir_rows, results_dir),
        "slack": export_slack_lbbd(model, data, results_dir),
        "lbbd_master_solution": export_master_solution_lbbd(model, results_dir),
    ...
    notes.to_csv(results_dir / "model_notes.csv", index=False)
    frames["model_notes"] = notes
