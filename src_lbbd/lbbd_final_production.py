...
from .lbbd_logging import ensure_dir, technology_suffix, tee_console_to_file, write_json
...
def _clean_run_dir(project_root: Path, dataset: str, scenario: str, disable_pv: bool, disable_bess: bool) -> Path:
    """Create the final thesis-scale LBBD run folder."""
...
def _preset_values(preset: str | None, dataset: str) -> dict[str, Any]:
    preset = preset or ("full_trial" if dataset == "full" else "small_validation")
    if preset == "full_trial":
        return {
            "master_gap": 0.005,
...
def _quality_checks(summary: dict[str, Any], iter_last: dict[str, str], target_lbbd_gap: float, disable_pv: bool = False, disable_bess: bool = False) -> list[dict[str, Any]]:
    ...
    if disable_pv:
        pv_used = _as_float(summary.get("pv_used_total_kWh")) or 0.0
        add(
            "disabled_pv_is_zero",
            (pv is not None and abs(pv) <= 1e-5 and abs(pv_used) <= 1e-3),
            f"PV_panels={pv}, pv_used_total_kWh={pv_used}",
            "PV_panels <= 1e-5 and pv_used_total_kWh <= 1e-3",
            "required because --disable-pv was passed",
        )

    if disable_bess:
        grid_batt = _as_float(summary.get("grid_to_battery_kWh")) or 0.0
        pv_batt = _as_float(summary.get("pv_to_battery_kWh")) or 0.0
        batt_dis = _as_float(summary.get("battery_discharge_kWh")) or 0.0
        add(
            "disabled_bess_is_zero",
            (batt is not None and abs(batt) <= 1e-5 and abs(grid_batt) <= 1e-3 and abs(pv_batt) <= 1e-3 and abs(batt_dis) <= 1e-3),
            f"Batt={batt}, grid_to_batt={grid_batt}, pv_to_batt={pv_batt}, batt_discharge={batt_dis}",
            "Batt <= 1e-5 and all BESS energy flows <= 1e-3 kWh",
            "required because --disable-bess was passed",
        )

    return rows


def _technology_label(disable_pv: bool, disable_bess: bool) -> str:
...
def _final_summary_row(run_dir: Path, dataset: str, scenario: str, elapsed: float, disable_pv: bool = False, disable_bess: bool = False) -> dict[str, Any]:
    results = run_dir / "results"
    summary = read_model_summary(results)
    iter_last = _read_last_csv_row(results / "lbbd_iteration_history.csv")
    tech_label = _technology_label(disable_pv, disable_bess)

    gap = _as_float(iter_last.get("lbbd_gap"))
    return {
        "dataset": dataset,
        "scenario": scenario,
        "technology_case": tech_label,
        "formulation": f"LBBD_{tech_label}_type_aware_redirection",
        "run_dir": str(run_dir),
        ...
        "final_ub_SEK": iter_last.get("master_objective_UB_SEK") or iter_last.get("final_ub_SEK") or "",
...
def _postprocess_clean_outputs(run_dir: Path, dataset: str, scenario: str, elapsed: float, target_lbbd_gap: float, disable_pv: bool = False, disable_bess: bool = False) -> tuple[dict[str, Any], list[dict[str, Any]]]:
...
def run_final_lbbd(args: Any) -> int:
    ...
    run_dir = _clean_run_dir(project_root, args.dataset, args.scenario, disable_pv, disable_bess)
    log_path = run_dir / "README_RUN.txt"

    with tee_console_to_file(log_path):
        try:
            return _run_final_lbbd_impl(
                args=args,
...
def _run_final_lbbd_impl(
    ...
    print("========== FINAL LBBD PRODUCTION RUN ==========")
    ...
    print(f"Preset         : {args.preset or ('full_trial' if args.dataset == 'full' else 'small_validation')}")
    ...
    print("Output logic   : one clean LBBD run folder")
    ...
    print("===============================================")
    print(f"Run folder     : {run_dir}")
    print(f"Terminal log   : {log_path.name}")
    ...
    write_json(
        run_dir / "logs" / "final_manifest.json",
        {
            "workflow": "final_lbbd_production",
            "dataset": dataset,
            ...
            "note": "Final thesis-facing LBBD runner with optional PV/BESS switches and type-aware redirection recourse.",
        },
    )
    ...
    summary_row, quality = _postprocess_clean_outputs(run_dir, dataset, args.scenario, elapsed, float(values["lbbd_gap"]), disable_pv=disable_pv, disable_bess=disable_bess)

    failed = [r for r in quality if not r["passed"]]
    manifest_path = run_dir / "logs" / "final_manifest.json"
    ...
    print("Summary file        : results/final_run_summary.csv")
    print("Quality checks      : results/final_quality_checks.csv")
    print(f"Terminal log        : {log_path.name}")
    print("========================================")
