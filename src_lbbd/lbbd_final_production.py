...
from .lbbd_stage4_loop import run_stage4_lbbd
...
def _clean_run_dir(project_root: Path, dataset: str, scenario: str, disable_pv: bool, disable_bess: bool) -> Path:
    """Create the single final thesis-scale LBBD run folder.

    The name intentionally avoids the development-stage labels. The mathematical
    model underneath is the validated PV+BESS LBBD production formulation
    """
...
def _quality_checks(summary: dict[str, Any], iter_last: dict[str, str], target_lbbd_gap: float) -> list[dict[str, Any]]:
    ...
    add("soc_month_link_gap_zero", soc_gap is not None and abs(soc_gap) <= 1e-6, soc_gap, "<= 1e-6 kWh")
...
def _final_summary_row(run_dir: Path, dataset: str, scenario: str, elapsed: float) -> dict[str, Any]:
...
def _postprocess_clean_outputs(run_dir: Path, dataset: str, scenario: str, elapsed: float, target_lbbd_gap: float) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    # More user-facing aliases for the final thesis workflow.
    notes_src = run_dir / "results" / "model_summary_notes.csv"
    if notes_src.exists():
        shutil.copy2(notes_src, run_dir / "results" / "model_notes.csv")
    ...
    summary_row = _final_summary_row(run_dir, dataset, scenario, elapsed)
...
def run_final_lbbd(args: Any) -> int:
    ...
    if disable_bess:
        print("Warning: final thesis workflow is intended for PV+BESS. You passed --disable-bess, so BESS will be disabled.")
    ...
    run_dir = _clean_run_dir(project_root, dataset, args.scenario, disable_pv, disable_bess)
    ...
    write_json(
        run_dir / "logs" / "final_manifest.json",
        {
            ...
            "note": "This final runner uses the validated mathematical implementation internally.",
        },
    )

    t0 = time.time()
    result = run_stage4_lbbd(
        data=data,
        cfg=model_cfg,
        solver_cfg=solver_cfg,
        run_dir=run_dir,
    ...
    summary_row, quality = _postprocess_clean_outputs(run_dir, dataset, args.scenario, elapsed, float(values["lbbd_gap"]))
