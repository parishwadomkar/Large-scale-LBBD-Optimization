...
```powershell
python src_benders\run_benders.py `
  --dataset small `
...
Compare against monolithic and LBBD outputs:

```powershell
python src\compare_runs.py `
  --monolithic-run "runs\<MONOLITHIC_RUN_FOLDER>" `
  --benders-run "runs\<BENDERS_RUN_FOLDER>" `
  --lbbd-run "runs\<LBBD_RUN_FOLDER>"
```

Regenerate figures for a completed Benders run:

```powershell
python src\visualize_results.py `
  --run-dir "runs\<BENDERS_RUN_FOLDER>"
```
